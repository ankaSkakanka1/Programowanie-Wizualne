...
public partial class MainWindow : Window
{
    ...
    {
       TypeCombo.ItemsSource = new List<string>
     {
        "DNA",
        "RNA",
        "Białko",
    ...
    private void SamplesList_SelectionChanged(
        object? sender,
        ...
        RoutedEventArgs e)
    {
        ...
        {
            SetStatus(
                "Próbka o podanym ID już istnieje.");
        ...
        RoutedEventArgs e)
    {
        ...
        if (_databaseService.SampleExists(
            sample.Id,
            _selectedSampleId))
        {
            SetStatus(
                "Podane ID jest już zajęte przez inną próbkę.");

            return;
        }

        _databaseService.UpdateSample(
            _selectedSampleId,
            sample);
    ...
    RoutedEventArgs e)
{
    try
    {
        if (SamplesList.SelectedItem is not Sample sample)
        {
            SetStatus("Wybierz próbkę do usunięcia.");
            return;
        }

        _databaseService.DeleteSample(sample.Id);
    ...
    {
        SetStatus($"Błąd usuwania: {ex.Message}");
    }
}
        ...
        RoutedEventArgs e)
    {
        ...
        printDocument.PrintPage +=
            (s, ev) =>
            {
                ...
                ev.Graphics.DrawString(
                    $"Data: {DatePicker.SelectedDate:yyyy-MM-dd}",
                    font,
                    Brushes.Black,
                    20,
                    350);
    ...
    private Sample? ReadSampleFromForm()
    {
        ...
        {
            SetStatus(
                "Wypełnij pola ID, nazwa i typ próbki.");
    ...
    private static byte[] CreateQrCodeBytes(
        Sample sample)
    {
        var text =
            $"ID:{sample.Id}\n" +
